        ...
        @isset($pricing_detail[1])
            ...
            <div class="col-lg-3 col-sm-6">
                <div class="ab-single-organic-package ab-single-organic-package-fourth">
                    ...
                    @endif
                    <div class="pkg-button ab-organic-btn text-center">
					 <button type="button" value="{{ $pricing_detail[1]->id }}" class="{{ $btn_class}} doddle-btn fill"><span></span><b></b>Order Now
					 </button>                       
                    </div>
                </div>
            </div>
        ...
        @isset($pricing_detail[2])
            ...
            <div class="col-lg-3 col-sm-6">
                <div class="ab-single-organic-package ab-single-organic-package-second">
                    ...
                    @endif
                    <div class="pkg-button ab-organic-btn text-center">                    	
					 <button type="button" value="{{ $pricing_detail[2]->id }}" class="{{ $btn_class}} doddle-btn fill"><span></span><b></b>Order Now
					 </button>
                    </div>
                </div>
            </div>
                ...
                <div class="ab-single-organic-package ab-single-organic-package-third">
                    <div class="ab-single-organic-head text-center">
                        ...
                        <h1>{{ SM::get_setting_value('currency') }} {{ $spd->price }}
                            /{{SM::getPackagePaymentTypeName($spd->price_type)}}</h1>
                        <!-- <h1>${{ $spd->price }}</h1> -->
                        <p>{{ $subtitle['premium_pricing_subtitle'] }}</p>
                    </div>
                    <?php
                    $details = is_string( $spd->details ) && $spd->details != '' ?
                        json_decode( $spd->details ) : [];
                    ...
                    @if(count($details)>0)
                        @foreach($details as $detail)
                            <div class="organic-phage-content text-center">
                                <h4 class="phage-title">{{ $detail->premium }}</h4>
                                @if(isset($detail->sub) && count($detail->sub)>0)
                                    @foreach($detail->sub as $single)
                                        <p>
                                            {!! $single->premium !!}
                    ...
                    @endif
                    <div class="pkg-button ab-organic-btn text-center">
					 <button type="button" value="{{ $pricing_detail[3]->id }}" class="{{ $btn_class}} doddle-btn fill"><span></span><b></b>Order Now
					 </button> 
                    </div>
                </div>
            </div>
