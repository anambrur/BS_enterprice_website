...
class Checkout extends Controller {
	public function index( $id, $detailId = null ) {
		...
		if ( count( $packageInfo ) > 0 ) {
			...
			$packageInfo->detail  = Package_detail::whereIn( "id", $selectedPackageDetailIds )
			                                      ...
			                                      ->get();
			$data["package_name"] = $packageInfo->title;
...
//							$pricing_title .= $pcd->title . ", Words: " . $pcd->required_words . ", Quantity: " . $pcd->qty;
			...
			$data["total_checkout"] = $data["amount"] + $data['tax'];
...
//			$clientSecret = 'EF93DMB6AV5MIizVektFJMG9dvBDqBOsdjc4JPE4D_xFLYWKuqQYoUClnUhllnTkS7LsL5r1uA5b3dbE';
	...
	private function paypalOrder( $isSaveOrder, $packageInfo, $session, $discount = 0, $request ) {
		...
		if ( $paymentMethodId == 1 || $paymentMethodId == 2 || $paymentMethodId == 3 ) {

			/**
			 * if payment type not equal to fixed then go to agreement section
			 */
//			if ( $packageInfo->type != 4 && isset( $packageInfo->detail[0] ) && $packageInfo->detail[0]->price_type != 1 ) {
//				return $this->paypalAgreement( $isSaveOrder, $packageInfo, $session, $discount, $request );
//			}
...
//				exit();
	...
	public function saveOrder( OrderValidation $request ) {
		...
		if ( $request->hasFile( 'attachments' ) ) {
			$session["attachments"] = SM::sm_multiple_file_upload( $request->file( 'attachments' ) );
		}
		...
		return $this->paypalOrder( 1, $packageInfo, $session, $discount, $request );
	}
	...
	private function saveOrderInfo() {
		if ( Session::has( "checkout" ) ) {
			...
			Auth::user()->firstname = $session["firstname"];
			Auth::user()->lastname  = $session["lastname"];
			Auth::user()->street    = $session["street"];
			Auth::user()->city      = $session["city"];
			Auth::user()->zip       = $session["zip"];
			Auth::user()->country   = $session["country"];
			Auth::user()->state     = $session["state"];
			Auth::user()->update();
			...
			if ( $order->package_type == 1 ) {
				$order->package_detail_type = isset( $session["package_detail_type"] ) ? $session["package_detail_type"] : 1;
			}
			if ( $packageInfo->type == 2 ) {
				$order->rate = 0;
			} else {
				$order->rate = isset( $session["amount"] ) ? $session["amount"] : 0;
			}
			$order->total         = isset( $session["total"] ) ? $session["total"] : 0;
			...
			$order->contact_email = isset( $session["contact_email"] ) ? $session["contact_email"] : "";
			$order->message       = isset( $session["message"] ) ? $session["message"] : "";
			$order->attachments   = isset( $session["attachments"] ) ? $session["attachments"] : "";
			$order->save();
			...
			if ( count( $packageInfo->detail ) > 0 ) {
				foreach ( $packageInfo->detail as $p_detail ) {
					$orderDetail                    = new Order_detail();
					$orderDetail->order_id          = $order->id;
					$orderDetail->package_detail_id = $p_detail->id;
	...
	public function pay( $id ) {
		...
		$order = Order::with( 'package', 'payment', 'user', 'detail' )
		              ->find( $id );
		if ( count( $order ) > 0 ) {
