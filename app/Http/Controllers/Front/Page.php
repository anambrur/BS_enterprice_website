...
//
	...
	public function subscribe( Request $request ) {
		...
		Mail::to( $request->email )
		    ->queue( new SubscribeMail( (object) $request->all() ) );
		...
		if ( $isHtmlReturn ) {
			if ( count( $packages ) > 0 ) {
				foreach ( $packages as $package ) {
					$list .= '<a  class="list-group-item" target="_blank" href="' . url( 'packages/detail/' . $package->slug ) . '">';
					$list .= $package->title;
					$list .= '</a>';
				}
			}
		...
		if ( $isHtmlReturn ) {
			if ( count( $services ) > 0 ) {
				foreach ( $services as $service ) {
					$list .= '<a  class="list-group-item" target="_blank" href="' . url( 'services/detail/' . $service->slug ) . '">';
					if ( $service->image != '' ) {
		...
		if ( $isHtmlReturn ) {
			...
			if ( count( $cases ) > 0 ) {
				foreach ( $cases as $case ) {
					...
					$list .= '<a  class="list-group-item" target="_blank" href="' . url( 'cases/detail/' . $case->slug ) . '">';
					if ( $case->image != '' ) {
						$img = $case->image;
						if ( ! $image_array = explode( ',', $img ) ) {
							$image_array[0] = array( $img );
