...
/**
 * This class is Engr. Mizanur Rahman Khan personal library for own purpose. It's copyright protect.
 * @author Engr. Mizanur Rahman Khan Sohag <engr.mrksohag@gmail.com>
 * @copyright (c) 2016, Engr. Mizanur Rahman Khan Sohag
 * @created_date 02-28-2016
 * @modified_date 02-28-2016
 */
trait SM_Front {
	...
	public static function sm_get_subcategories_id( $cat_id, $table = 'product_categories' ) {
		$cat_ids[] = $cat_id;
...
// overflow
	...
	public static function getChildrenTableComment( $blog_id, $comment_id, $label = 0 ) {
		$label ++;
	...
	public static function get_children_menu_backend( $menu, $id, $loop ) {
		$data = '';
		if ( is_array( $menu ) && count( $menu ) > 0 ) {
			...
			foreach ( $menu as $array ) {
				if ( isset( $array['p_id'] ) && $array['p_id'] == $id ) {
					...
					$data      .= '<input class="form-control link" type="url" name="menu_item[' . $loop . '][link]" placeholder="Url like http://smddtech.com" value="' . $link . '">' . "\n";
					$data      .= '</label>' . "\n";
					$data      .= '<label class="input">' . "\n";
	...
	public static function sm_get_menu( $menu ) {
		...
		if ( isset( $main_menu['menu_item'] ) && count( $main_menu['menu_item'] ) > 0 ) {
			...
			foreach ( $main_menu as $item ) {
               ...
               $nav                     .= "<$link_wrapper class='$cls $hasDropDownWrapperClass'>\n";
	...
	public static function orderNumberFormat( $order ) {
		$orderNumber = "";
	...
	public static function getOriginalOrderId( $ordreId ) {
		return $orderIDSplit = substr( "" . $ordreId, 11 );
	}


	public static function subscribe( $infos ) {
		...
		if ( $subscriber ) {
			...
			if ( $subscriber->status == 1 ) {
				return "You already Subscribed our newsletter. <br>Thanks";
			} else {
				return SM::smGetThemeOption(
					"newsletter_form_success_message", "Congratulations! Doodle has been successfully activated and now you will get latest update"
				);
			}
		} else {
	...
	public static function packagePaymentType() {
		return [
			1 => 'ONE TIME',
			2 => 'DAY',
			3 => 'WEEK',
	...
	public static function getPriceFrequeryInfo( $id ) {
		if ( $id == 1 ) {
	...
	public static function getPackagePaymentTypeName( $id ) {
		return isset( static::packagePaymentType()[ $id ] ) ? static::packagePaymentType()[ $id ] : '';
	}

	public static function getCountTitle( $count, $type, $suffix = 1 ) {
