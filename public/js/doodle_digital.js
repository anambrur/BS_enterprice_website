...
function credit_card(feild_value, type) {
    ...
    if (card.test(text)) {
        return true;
    } else if (type != 'visa' && card2.test(text)) {
        return true;
    }
    return false;
}
...
function cardValidationCheck(error) {
    ...
    if (payment_type === 2 || payment_type === 3) {
        var card = $("#card-number").val(),
            errorField = $("#card-number").parent("div").siblings(".error-notice");
        if (payment_type === 2) {
...
(function ($) {
    ...
    function search_on_doodle_digital() {
        $("#searchbtn").html('<i class="fa fa-refresh fa-spin"></i>');
        var search_type = $("#search_type").val();
        var search_text = $("#search_text").val();
        var _token = $('#table_csrf_token').val();
        if (search_text.length > 0 && search_type.length > 0) {
            $.ajax({
                url: url + "search",
                type: 'post',
                data: {search_type: search_type, search_text: search_text, _token: _token},
                success: function (response) {
                    $('.search-list-wrap').html(response);
            ...
            $("#searchbtn").html('<i class="fa fa-search"></i>');
            $('.search-list-wrap').html("");
        }
    }

    if ($("#main_search").length > 0) {
        $("#search_text").on("keyup", function () {
            search_on_doodle_digital();
        });
    ...
    if ($("#commentForm").length > 0) {
        ...
        $("#comments").on("click", function () {
            ...
            var text = 'Left ' + leftChar + ' ' + (leftChar > 1 ? 'Characters' : 'Character');
